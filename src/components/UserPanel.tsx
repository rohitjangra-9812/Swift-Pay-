...
export const UserPanel = ({ 
  ...
  const handleProcessPayment = () => {
    ...
    setTimeout(() => {
      ...
      // Wait for user to close manually via onClose in PaymentSuccess
    }, 200);
  };
  ...
  return (
    <div className={`w-full h-full text-white relative pb-20 overflow-y-auto overflow-x-hidden transition-colors duration-500 ${getThemeClasses()}`}>
      ...
      {/* Action Overlay */}
      {activeAction && (
        <div className="absolute inset-0 z-50 bg-slate-950/90 backdrop-blur-sm p-6 animate-in slide-in-from-bottom-full duration-300 overflow-y-auto overflow-x-hidden">
            ...
            <div className="flex flex-col items-center py-8 min-h-full">
              {paymentSuccess ? (
                <PaymentSuccessComponent 
                  amount={amount} 
                  recipient={scannedData ? scannedData.merchant : activeAction?.label} 
                  onClose={() => {
