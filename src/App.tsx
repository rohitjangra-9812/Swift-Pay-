...
export default function App() {
  ...
  useEffect(() => {
    if (setupStep === "otp" && "OTPCredential" in window) {
      ...
      .then((otp: any) => {
        if (otp && otp.code) {
          console.log("[App] Web OTP API successfully retrieved code:", otp.code);
          setSetupOtp(otp.code);
          toast.success("OTP auto-filled from SMS");
        }
      })
      .catch((err) => {
        console.error("[App] Web OTP API Error:", err);
      });
  ...
  const handleSetupPin = async (e: React.FormEvent) => {
    ...
    if (setupStep === 'mobile') {
      ...
      if (setupMobile.length === 10) {
        ...
        try {
          const response = await SmsService.sendSms({
            to: `+91 ${setupMobile}`,
            message: `${otp} is your SwiftPay registration code.\n\n@${window.location.host} #${otp}`
          });
          console.log(`[App] SmsService response:`, response);
          setSetupStep('otp');
          toast.success(`OTP sent to ${setupMobile}`);
        } catch (err) {
          console.error(`[App] Exception caught during sendSms:`, err);
          toast.error('Network error while sending OTP. Check console for details.');
        } finally {
          setIsSendingSms(false);
        }
      } else {
        console.warn(`[App] Invalid mobile number length: ${setupMobile.length}`);
        toast.error('Please enter a valid 10-digit mobile number.');
      }
    } else if (setupStep === 'otp') {
      console.log(`[App] Verifying OTP. Entered: ${setupOtp}, Expected: ${expectedOtp}`);
      if (setupOtp === expectedOtp) {
        setSetupStep('pin1');
        toast.success('Mobile verified successfully!');
      } else {
        console.warn(`[App] OTP mismatch!`);
        toast.error('Invalid OTP. Please check your SMS and try again.');
      }
    } else if (setupStep === 'pin1') {
      console.log(`[App] Setting initial PIN...`);
      if (setupPin.length === 4) setSetupStep('pin2');
    } else {
      console.log(`[App] Confirming PIN...`);
      if (setupPin === confirmPin) {
        ...
        console.log(`[App] Registration complete. App unlocked.`);
        toast.success('Registration complete & App PIN set successfully!');
      } else {
        console.warn(`[App] PIN confirmation mismatch!`);
